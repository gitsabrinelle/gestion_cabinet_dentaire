...
$id_facture;
$ref_client;
if(isset($_GET['id_bl'])&& isset($_GET['id_facture'])&&
 isset($_GET["ref_client"]))
{ 
...
//echo("$id_bl");
$requete_montant=mysqli_query($link,"select * from facture where id_facture='".$id_facture."'")or die(mysqli_error($link));
$row_requete_montant = mysqli_fetch_array($requete_montant);
$montant_facture = $row_requete_montant["montant_facture"];
$Montant = $row_requete_montant["montant_ht"];
...
$reste_facture = $quantite_total-$quant_comptab;

	if ($quantite-$reste_facture>0)
	$quantite=$reste_facture;
...
if ($quantite>0 && $montant_facture-$Montant>=0)	
{

$req_quant=mysqli_query($link,"select * from detail_facture where id_article = '".$id_article."' and id_facture = '".$id_facture."' ")or die (mysqli_error($link));

$row_quant=mysqli_fetch_array($req_quant);
$qtt=$row_quant["quantite"];

if (mysqli_num_rows($req_quant)==0)
{
...
{
$quantite=$qtt+$quantite;	
	mysqli_query($link,"UPDATE `dap2`.`detail_facture` SET `quantite` = '".$quantite."',Montant = '".$montant*$quantite."',prix='".$montant."' WHERE `detail_facture`.`id_article` = '$id_article' and id_facture = '".$id_facture."'")or die (mysqli_error($link));
	
	}

}//else
	//die ("la Transformation de la Facture a été bien fait");
	}
	}
...
$sql33 = mysqli_query($link,"UPDATE facture SET montant_ht = '$total_facture_ht' WHERE id_facture = '$id_facture';")or die(mysqli_error($link));
	echo("la Transformation de la Facture a été bien fait"); 
    $tri;
  $lien;
  if (isset($_GET["tri"]))
	 {
	 $tri = $_GET["tri"];
  $lien= "?id_facture=".$id_facture."&ref_client=".$ref_client."&tri=".$tri;
  }
  
  else
{	 
$lien= "?id_facture=".$id_facture."&ref_client=".$ref_client;
} 
 //header("Location:../factures/facture-vente.php".$lien);  
 ?> </center></b>
	<?php
?>
</div>
