...
{}
	...
	$sqlquery = mysqli_query($link,"SELECT * 
FROM  `facture_achat` ")or die (mysqli_error($link));
	if (mysqli_num_rows($sqlquery))
	{
		$add_achat = 1;
		
		$reference = $_GET["inser_produit"];
	$request = mysqli_query($link,"INSERT INTO articles (`reference` , `ref_fournisseur`  ) VALUES ( '$desi',    (SELECT MIN( ref_fournisseur ) 
FROM fournisseurs
)
...
$request = mysqli_query($link,"
		INSERT INTO  `dap2`.`detail_fachat` (
...
VALUES (
 ...
 '".$prixunit."',
   (SELECT MIN( facture_achat.id_fachat ) FROM  facture_achat    where   facture_achat.ref_fournisseur = (SELECT MIN( ref_fournisseur ) FROM fournisseurs) ),  
  '2', 
'$id_user'
); " ) or die( mysqli_error($link) );
// die ("prix ".$prixunit); 
